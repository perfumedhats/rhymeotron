...
/**
 * This gives the angle between two vectors, as a measure of similarity
...
function cosineSimilarity(x,y) {
    ...
    // A for loop here is slightly faster than reduce
    let acc = 0;
    for (let i = 0; i < 25; i++) {
...
function comparePhonemes (x, y) {
    // Phoneme vectors can have a few values, + for present, - for absent, r for reduced, and X for unapplicable.
    ...
    // e.g. p and b have a similarity of .92, whereas k and aɪ are only .5
...
function lookupWord (target) {
    // This could be replaced with binary search, but it isn't on the hot-path
    return phonology.words.filter(x=>x.word==target)[0];
}

function compareWords (word1, word2) {
    // TODO I could use the harmonic mean
